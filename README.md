...
public:
    bool isValidBST(TreeNode* root) {
        std::vector<int> orderVals;
        if(!isValidBST(root, orderVals))
            return false;
        
        for(int i = 0; i < orderVals.size()-1; ++i)
        {
            if(orderVals[i] >= orderVals[i+1])
                return false;
        }
        return true;
    }
    
    bool isValidBST(TreeNode* root, std::vector<int>& orderVals)
    {
        auto val = root->val;
        ...
        if(leftNode)
        {
            if(auto leftNodeVal = leftNode->val; leftNodeVal)
            {
                if(leftNodeVal >= val)
                    return false;
                if(!isValidBST(leftNode, orderVals))
                    return false;
            }
        }

        orderVals.push_back(val);

        if(rightNode)
        {
            if(auto rightNodeVal = rightNode->val; rightNodeVal)
            {
                if(rightNodeVal <= val)
                    return false;
                if(!isValidBST(rightNode, orderVals))
                    return false;
            }   
        }
